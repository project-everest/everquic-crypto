...
let supported_cipher = function
  | C.AES128 | C.AES256 | C.CHACHA20 -> true
  | _ -> false

type ha = a:HD.hash_alg{supported_hash a}
type ea = a:AE.alg{supported_aead a}
type ca = a:C.cipher_alg{supported_cipher a}

val id: eqtype
inline_for_extraction val is_honest: id -> bool
val id_ginfo: i:id -> GTot (ha * ea * ca)
val id_info: i:id{model} -> a:(ha * ea * ca){a == id_ginfo i}

val ae_id: eqtype
inline_for_extraction val is_ae_honest: ae_id -> bool
val ae_id_ginfo: i:ae_id  -> GTot ea
val ae_id_info: i:ae_id{model} -> a:ea{a == ae_id_ginfo i}

val pne_id: eqtype
inline_for_extraction val is_pne_honest: pne_id -> bool
