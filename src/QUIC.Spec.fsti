...
let reduce_pn
  (pn_len:nat2)
  (pn:nat62)
: Tot (npn:nat{npn < bound_npn pn_len})
= pn % (bound_npn pn_len)
...
val expand_pn :
  ...
  pn:nat62{in_window pn_len last pn /\ reduce_pn pn_len pn = npn}
...
| Success: 
  h: header ->
  plain: pbytes ->
  seqn: nat62 { if is_retry h then seqn == 0 else U32.v (packet_number h) == reduce_pn (U32.v (pn_length h) - 1) seqn } ->
  result
| Failure
...
val encrypt:
  ...
  seqn: nat62 {
    (~ (is_retry h)) ==> 
    U32.v (packet_number h) == reduce_pn (U32.v (pn_length h) - 1) seqn
  } ->
...
val lemma_encrypt_correct:
  ...
  (requires (
    (~ (is_retry h)) ==> (
      in_window (U32.v (pn_length h) - 1) last seqn /\
      U32.v (packet_number h) == reduce_pn (U32.v (pn_length h) - 1) seqn)
  ))
  (ensures (
    decrypt a k siv hpk last (dcid_len h)
      (encrypt a k siv hpk h seqn p)
    == Success h p (if is_retry h then 0 else seqn)
  ))
